...
**Resolved:** Next.js standalone output mode — `app/next.config.ts` now uses `output: "standalone"` and traces `prompts/**/*.md` plus `public/mock-images/**/*` into `.next/standalone` via `outputFileTracingIncludes`.
