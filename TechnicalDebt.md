...
**Resolved:** Error states now use shared semantic error tokens instead of hardcoded `bg-red-*` / `text-red-*` classes. Palette-specific tuning can be added later by overriding the shared tokens per palette if desired.
