...
**Resolved:** Error state colors are tokenized via shared semantic error tokens in `app/app/globals.css`, and all current error surfaces use those tokens instead of hardcoded `bg-red-*` / `text-red-*` classes.
