...
**Resolved:** Error state colors are tokenized via shared `--error`, `--error-soft`, `--error-border`, and `--error-contrast` palette variables in `app/app/globals.css`; removed from open debt.
