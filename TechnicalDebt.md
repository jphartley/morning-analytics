...
**Resolved:** Error state colors are now tokenized via semantic error theme tokens in `app/app/globals.css`; `ErrorState`, toast messaging, history load errors, and auth form errors now use palette-aware classes instead of hardcoded Tailwind red scales.
