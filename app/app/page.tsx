...
export default function Home() {
  ...
  return (
    <>
      ...
      {/* Save error toast */}
      {saveError && (
        <div className="fixed bottom-4 right-4 bg-error-soft border border-error-border text-error-contrast px-4 py-3 rounded-lg shadow-lg flex items-center gap-3">
          <span className="text-sm">{saveError}</span>
          <button
            onClick={() => setSaveError(null)}
            className="text-error hover:opacity-80 font-bold transition-opacity"
          >
            ×
          </button>
        </div>
